...
public struct IMDrawGLBatch
{
	private LinkedList<IMDrawGLCommand>		m_GLCommandList; // List for active GL draw commands

	private int								m_VertStartIndex;
	private int								m_VertEndIndex;

	public void Init ()
	{
		m_GLCommandList = new LinkedList<IMDrawGLCommand>();
		m_VertStartIndex = m_VertEndIndex = 0;
	}

	public void AddCommand (IMDrawGLCommand command)
	{
		m_GLCommandList.AddLast(command.m_ListNode);
	}

	public void FlushAll (Stack<IMDrawGLCommand> commandPool)
	{
		while (m_GLCommandList.Count > 0)
		{
			commandPool.Push(m_GLCommandList.Last.Value.Dispose());
			m_GLCommandList.RemoveLast();
		}
	}

	public void Update (Stack<IMDrawGLCommand> commandPool, float deltaTime)
	{
		...
		while (node != null)
		{
			...
			{
				nextNode = node.Next;
				commandPool.Push(node.Value.Dispose());
	...
	/// <summary>Build vertex data. Return true if we have reached vert limit.</summary>
	public bool BuildVertexData(
		Vector3[] vertexArray,
		Color[] colorArray,
		ref int vertIndex,
		int maxVertices)
	{
		m_VertStartIndex = vertIndex;
		...
		while (node != null)
		{
			...
			if ((vertIndex + command.m_Verts) <= maxVertices) // Check if there are enough verts left to render
			{
				node.Value.Draw(vertexArray, colorArray, ref vertIndex);

				node = node.Next;
			}
			else
			{
				m_VertEndIndex = vertIndex;
				return true; // We have run out of vertices
			}
		}
	...
	///<summary>Build vertex data with distance based culling. Return true if we have reached vert limit.</summary>
	public bool BuildVertexData (
		Vector3[] vertexArray,
		Color[] colorArray,
		ref int vertIndex,
		...
		float cullDistSqrd)
	{
		...
		while (node != null)
		{
			...
			if ((vertIndex + command.m_Verts) <= maxVertices) // Check if there are enough verts left to render
			{
				if (command.GetDistSqrd(ref cameraPos) < cullDistSqrd) // Check if the mesh is inside our culling range
				{
					node.Value.Draw(vertexArray, colorArray, ref vertIndex);
				}
	...
	public void Draw (
		...
		IMDrawZTest ztest)
	{
		material.SetInt(IMDrawSPID._ZTest, (int)ztest);

		material.SetPass(0);

		GL.Begin(GL.LINES);

		int vertex = m_VertStartIndex;

		while (vertex < m_VertEndIndex)
		{
			GL.Color(colorArray[vertex]);
			GL.Vertex(vertexArray[vertex]);
	...
	public int CommandCount { get { return m_GLCommandList.Count; } }

	public int VertexCount { get { return (m_VertEndIndex - m_VertStartIndex); } }
}
