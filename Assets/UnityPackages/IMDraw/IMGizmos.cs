...
public static class IMGizmos
{
	...
	private static readonly Quaternion		AXIS_X_ROTATION = Quaternion.Euler(-90f, -90f, 0f);
	...
	private static void DrawMesh(Mesh mesh, Vector3 position, Quaternion rotation, Vector3 scale, Color color)
	{
		...
		if (m_GizmoMaterial == null)
		{
			...
			m_ColorPropID = Shader.PropertyToID("_C");
		}
	...
	private static void SetRotation(ref Quaternion rotation, GizmoDrawAxis axis)
	{
#if UNITY_EDITOR
		switch (axis)
		{
			case GizmoDrawAxis.X: rotation = rotation * AXIS_X_ROTATION; break;
			//case IMDrawAxis.Y: outRotation = inRotation; break;
			case GizmoDrawAxis.Z: rotation = rotation * AXIS_Z_ROTATION; break;
	...
	public static void Label(float x, float y, Color color, LabelPivot pivot, LabelAlignment alignment, string label, int fontSize = 12)
	{
		...
		if (x > camera.pixelWidth || y > screenHeight)
			return;
	...
	public static void Label (Vector3 position, Color color, LabelPivot pivot, LabelAlignment alignment, string label, float maxDist, int fontSize = 12)
	{
		...
		UnityEditor.Handles.BeginGUI();
		GUI.Label(new Rect((float)System.Math.Round(screenPos.x), (float)camera.pixelHeight - (float)System.Math.Round(screenPos.y), size.x, size.y), m_GUIContent, m_Style);
		UnityEditor.Handles.EndGUI();
#endif // UNITY_EDITOR
	}
	...
	public static void Label(Vector3 position, float offsetX, float offsetY, Color color, LabelPivot pivot, LabelAlignment alignment, string label, float maxDist, int fontSize = 12)
	{
		...
		UnityEditor.Handles.BeginGUI();
		GUI.Label(new Rect((float)System.Math.Round(screenPos.x + offsetX), (float)camera.pixelHeight - (float)System.Math.Round(screenPos.y - offsetY), size.x, size.y), m_GUIContent, m_Style);
		UnityEditor.Handles.EndGUI();
#endif // UNITY_EDITOR
	}
	...
	public static void Image2D (Rect rect, Vector2 pivot, float angle, Color color, Texture2D texture)
	{
		...
		if (angle != 0f)
		{
			...
			pivot.x = Screen.width / 2f;
			pivot.y = Screen.height / 2f;
